 ...
 */
static void SHA2_starts(SHA2_context * ctx, sha2_block_e block_size)
{
        ctx->total[0] = 0;
        ctx->total[1] = 0;
 ...
 */
void SHA2_hmac_starts(SHA2_context * ctx, const unsigned char *key, int keylen, sha2_block_e block_size)
{
        int                     i;
 ...
 */
void SHA2_hmac_finish(SHA2_context *ctx, unsigned char output[64])
{
        int                     block_size, hlen;
...
{
        SHA2_starts(ctx, ctx->block_size);
        SHA2_update(ctx, ctx->ipad, 128);
}
               ...
               const unsigned char *input, int ilen, unsigned char output[64], sha2_block_e block_size)
{
        ...
        SHA2_hmac_starts(&ctx, key, keylen, block_size);
 ...
 */
static unsigned char    SHA2_test_buf[3][113] = {
        {"abc"},
...
static const unsigned char SHA2_test_sum[6][64] = {
        /*
...
static const unsigned char SHA2_hmac_test_sum[14][64] = {
        /*
...
{
        int                     i, j, buflen;
        sha2_block_e            blocksz;
        unsigned char           buf[1024];
        unsigned char           SHA2sum[64];
        SHA2_context            ctx;

        for(i = 0; i < 6; i++) {
                j = i % 3;
                blocksz = (i < 3)? ANONTOOL_SHA2_BLOCK_384 : ANONTOOL_SHA2_BLOCK_512;

                printf("  SHA-%d test #%d: ", blocksz, j + 1);

                SHA2_starts(&ctx, blocksz);
                ...
                if(memcmp(SHA2sum, SHA2_test_sum[i], blocksz/8) != 0) {
                        printf("failed\n");
                        return (1);
                }
        ...
        for(i = 0; i < 14; i++) {
                j = i % 7;
                blocksz = (i < 7)? ANONTOOL_SHA2_BLOCK_384 : ANONTOOL_SHA2_BLOCK_512;

                printf("  HMAC-SHA-%d test #%d: ", blocksz, j + 1);

                if(j == 5 || j == 6) {
                        memset(buf, '\xAA', buflen = 131);
                        SHA2_hmac_starts(&ctx, buf, buflen, blocksz);
                } else
                        SHA2_hmac_starts(&ctx, SHA2_hmac_test_key[j], SHA2_hmac_test_keylen[j], blocksz);

                SHA2_hmac_update(&ctx, SHA2_hmac_test_buf[j], SHA2_hmac_test_buflen[j]);

                SHA2_hmac_finish(&ctx, SHA2sum);

                buflen = (j == 4) ? 16 : blocksz/8;
