...
static int tcp_num[MAX_LIBNIDS_INSTANCES];
...
static void
free_tcp(struct tcp_stream * a_tcp,int id)
{
  ...
  if (a_tcp->prev_node)
    ...
    tcp_stream_table[id][hash_index] = a_tcp->next_node;
  ...
  if (a_tcp == tcp_oldest[id])
    tcp_oldest[id] = a_tcp->prev_time;
  if (a_tcp == tcp_latest[id])
    tcp_latest[id] = a_tcp->next_time;
  ...
  a_tcp->next_free = free_streams[id];
  free_streams[id] = a_tcp;
  tcp_num[id]--;
}
...
static void
add_new_tcp(struct tcphdr * this_tcphdr, struct ip * this_iphdr,int id)
{
  ...
  if (tcp_num[id] > max_stream) {
    struct lurker_node *i;
    tcp_oldest[id]->nids_state = NIDS_TIMED_OUT;
    for (i = tcp_oldest[id]->listeners; i; i = i->next)
      (i->item) (tcp_oldest[id], &i->data);
    free_tcp(tcp_oldest[id],id);
    nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_TOOMUCH, ugly_iphdr, this_tcphdr);
  }
  a_tcp = free_streams[id];
  if (!a_tcp) {
    fprintf(stderr, "gdb me ...\n");
    pause();
  }
  free_streams[id] = a_tcp->next_free;
  
  tcp_num[id]++;
  tolink = tcp_stream_table[id][hash_index];
  memset(a_tcp, 0, sizeof(struct tcp_stream));
  a_tcp->hash_index = hash_index;
  a_tcp->addr = addr;
  a_tcp->client.state = TCP_SYN_SENT;
  //antonat
  a_tcp->client.headers = a_tcp->server.headers = NULL;
  ...
  tcp_stream_table[id][hash_index] = a_tcp;
  a_tcp->next_time = tcp_latest[id];
  a_tcp->prev_time = 0;
  if (!tcp_oldest[id])
    tcp_oldest[id] = a_tcp;
  if (tcp_latest[id])
    tcp_latest[id]->prev_time = a_tcp;
  tcp_latest[id] = a_tcp;
}
	  ...
	  )
{
  ...
  if (!after(this_seq, EXP_SEQ)) {
    if (after(this_seq + datalen + (this_tcphdr->th_flags & TH_FIN), EXP_SEQ)) {
      ...
      add_from_skb(a_tcp, rcv, snd, (unsigned char *)data, datalen, this_seq,
	    ...
	    int *from_client,int id)
{
  ...
  for (a_tcp = tcp_stream_table[id][hash_index];
...
void clear_stream_buffers(int id)
{
  int i;
  ...
  for (i = 0; i < tcp_stream_table_size; i++) {
    for (a_tcp = tcp_stream_table[id][i];
         a_tcp;
         a_tcp = a_tcp->next_node) {
...
void process_tcp(u_char * data, int skblen,int id)
{
...
*/
  ...
  if (!(a_tcp = find_stream(this_tcphdr, this_iphdr, &from_client,id))) {
...
void nids_register_tcp(void (*x)) {
	nids_anon_register_tcp(x,0);
}

void nids_anon_register_tcp(void (*x),int id)
{
  struct proc_node *i;
...
int tcp_init(int size,int id)
{
  int i;
  if (!size) return 0;
  tcp_stream_table_size = size;
  tcp_stream_table[id] = malloc(tcp_stream_table_size * sizeof(char *));
  if (!tcp_stream_table[id])
    nids_params.no_mem("tcp_init");
  memset(tcp_stream_table[id], 0, tcp_stream_table_size * sizeof(char *));
  
  max_stream = 3 * tcp_stream_table_size / 4;
  streams_pool[id] = (struct tcp_stream *) malloc((max_stream + 1) * sizeof(struct tcp_stream));
  if (!streams_pool[id])
    nids_params.no_mem("tcp_init");
  ...
  memset(tcp_latest,0,MAX_LIBNIDS_INSTANCES*sizeof(struct tcp_stream *));
  memset(tcp_oldest,0,MAX_LIBNIDS_INSTANCES*sizeof(struct tcp_stream *));
...
void
process_icmp(u_char * data,int id)
{
  ...
  if (!(a_tcp = find_stream(th, orig_ip, &from_client,id)))
    return;
