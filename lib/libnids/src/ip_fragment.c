...
ip_defrag_stub(struct ip *iph, struct ip **defrag,int id)
{
  ...
  if ((*defrag = (struct ip *)ip_defrag((struct ip *) (skb->data), skb,id)))
    return IPF_NEW;
...
ip_frag_init(int n,int id)
{
	...
	timer_head[id]=timer_tail[id]=0;
	
 	fragtable[id] = (struct hostfrags **) malloc(n * sizeof(struct hostfrags *));
  	if (!fragtable[id])
  		  nids_params.no_mem("ip_frag_init");
  	memset(fragtable[id], 0, n * sizeof(struct hostfrags *));
